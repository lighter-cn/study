    ...
    public function other () {
        ...
        ************************************************** */
        ...
        /* オブジェクト指向プログラミングの基本
            ...
            クラス :
                互いに関連するデータと、データを操作するメソッドを一つのオブジェクトと呼ばれる単位に一体化（カプセル化）して取り扱う
                そのオブジェクトがどのようなデータとメソッドから作られるのかを定義した雛形にあたるものがクラス
            インスタンス（オブジェクト） :
                クラスを元に作られたコピーをインスタンスまたはオブジェクトと呼び、その作業をインスタンス化という
                インスタンス化にはnew演算子を利用する
            ...
            オブジェクトのメソッドやプロパティは->で呼び出せる
            PHP8.0以降では?->と言うnull安全演算子が追加され、オブジェクトが非nullの時だけそのメンバーにアクセスすると言う状況に対応できるようになった

            静的メソッド / 静的プロパティ :
                例外的にオブジェクトを生成せずに呼び出せるメソッドやプロパティ
                クラスメソッドやクラスプロパティとも呼ばれる

                [戻り値 =] クラス名::メソッド名([引数,...]);
                クラス名::プロパティ名[= 値];

                また、これらと対照的にインスタンス経由で呼び出すメソッドやプロパティをインスタンスメソッド、インスタンスプロパティと呼ぶ
        */
